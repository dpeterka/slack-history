...
func TestGetRandomFunFact(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			fact := GetRandomFunFact(tc.includeEmo, tc.includeBlobby, tc.includeWikiHow, tc.includeWikiHowQuizzes, tc.includeQuote, false, false, false, false, false, false, false, false)
...
func TestGetRandomFunFactConsistency(t *testing.T) {
	...
	fact1 := GetRandomFunFact(true, true, true, false, false, false, false, false, false, false, false, false, false)
	fact2 := GetRandomFunFact(true, true, true, false, false, false, false, false, false, false, false, false, false)
...
func TestGetDisplayTitle(t *testing.T) {
	testCases := []struct {
		...
		{"foodtakes", "🌭 Unhinged Food Take of the Day"},
		{"joke", "💡 Fun Fact"},
		{"unknown", "💡 Fun Fact"},
	}
